# Regime 3 · 7misspec / 7-condition Constraint Sensitivity 说明

对应代码：`simulation0701_regime3_7misspec.ipynb`
...
**不做任何 BL reverse 条件。** Regime 3 结构为：
...
节点数 `p=16`。固定：`BASE_DIR <- "/content/drive/MyDrive/CBN_simulation_2"`，`SEED_BASE <- 2026`。

## 2. 七个 conditions

| 条件 | 定义 |
|---|---|
| `strong` | Regime 3 正确 strong blacklist baseline |
...
```text
/content/drive/MyDrive/CBN_simulation_2/outputs/simulation0701_regime3_7misspec/
├── row_results/
├── metadata/
├── true_dags/
├── strength_tables/
...
需要具体 replicate 时看 `all_results_raw.csv`；需要解释错误先验实际改了什么边时，必须看 `constraint_changes/` 和 `constraint_audits/`，并按 `edge_role` 汇总，例如 `A_to_B`、`B_to_C`、`A_to_Y`、`B_to_Y`、`C_to_Y`、`within_A/B/C`。
...
结论应按：**overall directed recovery → skeleton → constraint damage type → chain-edge role → rho / graph density / tau** 的顺序组织。
