...
合法跨 block 方向：`A -> B`、`B -> C`、`A -> Y`、`B -> Y`、`C -> Y`。当前版本不允许 `A -> C` 作为 skip-layer extra edge。所有 block 都允许 within-block true edges，并通过 acyclicity check 保证无环。
...
`target_edges = p * edge_multiplier`，因此 `edge_multiplier=1` 时 16 条边，`edge_multiplier=2` 时 32 条边。`WITHIN_TRUE_RATE <- 0.25` 控制 within-block true edges 的目标比例。
...
每个 job 只生成一次 true DAG、一次数值数据和一次 `dat_disc`；同一份 `dat_disc` 复用于所有 constraint conditions。学习继续使用 `hc + BDe`，`iss=10`。`rho` 沿用原参数名并控制 common-factor collinearity。
...
Regime 1 vs 3 是节点数相同的并列上游 vs 链式机制主比较；Regime 2 vs 4 是有 D block 时的对应比较。Regime 1 vs 2 和 3 vs 4 用于考察增加 D -> Y 的影响。
