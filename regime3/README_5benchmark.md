# Regime 3 · 5benchmark 说明

对应代码：`simulation0701_regime3_5benchmark.ipynb`
...
```text
/content/drive/MyDrive/CBN_simulation_2/outputs/simulation0701_regime3_5benchmark/
├── row_results/
├── metadata/
├── true_dags/
├── strength_tables/
...
Regime 3 还应重点判断：在 `A -> B -> C -> Y` 链式机制下，constraint 是否比 Regime 1 的并列上游结构更能改善方向恢复，尤其是 A -> B 和 B -> C 机制边。

## 5. 结论组织

按 **overall directed recovery → skeleton → chain-edge recovery → rho / graph density / tau** 组织。不要把 strong 的 `backward_edge_rate=0` 单独当作算法学习能力证据，应结合 TPR、F1 和 SHD 判断约束收益。
